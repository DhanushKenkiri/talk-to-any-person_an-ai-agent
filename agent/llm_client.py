...
class BedrockConversationalResponder:
    ...
    def answer(
        ...
        if not self.client:
            return self._fallback(name, query, results)
    ...
    def _fallback(self, name: str, query: str, results: list[SearchResult]) -> str:
        lines = [
            f"Hi, I'm {name}.",
            "I cannot answer that question because the conversational model is unavailable.",
